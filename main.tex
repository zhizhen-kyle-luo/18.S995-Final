...
We use \texttt{distilbert-base-uncased}, with $n_\ell=6$ transformer layers and hidden size $d=768$. We tokenize $120$ short English sentences and run the model with \texttt{output\_hidden\_states=True}, which yields seven hidden-state tensors: the embedding output (\textsf{embed}) and the outputs of layers $1$ through $6$ (\textsf{layer\_1}, \dots, \textsf{layer\_6}). For each hidden state $\ell$ we strip padding via the attention mask, concatenate non-padding rows across the batch, and subsample with a fixed seed to at most $512$ tokens. The result is a feature matrix $X_{\ell,\mathrm{raw}}\in\R^{n_\ell\times 768}$ with $n_\ell\le 512$.

\subsection{Core Transformations}

For each $X_{\ell,\mathrm{raw}}$ we form four transformed variants. The LayerNorm matrix is rowwise:
\begin{equation}
    X_{\ell,\LN}[i,:]
    =
...
We find that LayerNorm and DyT diverge sharply at the row-norm level but converge at the global spectral level. Figure~\ref{fig:spectrum-X} previews this: the normalized singular value decay of $X_{\ell,T}$ at the final layer is essentially identical across $T$, modulo a leading-coefficient scale. The remaining subsections isolate where, and how much, the maps actually differ.
...
The relative Frobenius error of the best rank-$32$ approximation \eqref{eq:low-rank-error} is $0.336$ (raw), $0.363$ (LN), $0.360$ (DyT $\alpha=0.25$), and $0.383$ (DyT $\alpha=1.0$): all five transforms lie within five percentage points of one another. Low-rank approximability is not a discriminating axis here; the diagnostic detects the differences in row geometry but the global spectral compressibility is shared.

\subsection{Attention Scores and Perturbation Response}
\label{sec:results-attention}

At a $1\%$ relative perturbation of $Q$ and $K$, the mean response $\|\Delta S\|_F/\|S\|_F$ is $0.005$--$0.010$ across all transforms and all transformer layers. The \textsf{layer\_6} averages are $0.0044$ (raw), $0.0056$ (LN), $0.0045$ (DyT $\alpha=0.25$), $0.0058$ (DyT $\alpha=0.5$), and $0.0074$ (DyT $\alpha=1.0$). The bound \eqref{eq:attention-relative-perturbation} predicts $\|\Delta Q\|_F/\|Q\|_F+\|\Delta K\|_F/\|K\|_F=2\%$ in the linear regime; the observed responses lie below this and do not separate transforms. Stable rank of the per-head score matrix shows a mild trend: $\srank(S)$ for DyT $\alpha=1.0$ exceeds LN by $\sim 0.3$--$0.6$ in middle layers and converges by \textsf{layer\_6}. Attention computed downstream of LN versus DyT does not exhibit a substantially different perturbation profile.

\section{Discussion}

The data answers the title question with a split. LN and DyT differ sharply in row-norm geometry: LN imposes exact row isometry on the hidden states, while DyT permits $\alpha$-dependent row magnitudes that vary with input. This is the cleanest empirical confirmation of Section~\ref{sec:maps} and the only diagnostic on which the two maps are unambiguously distinguishable. The remaining diagnostics --- effective rank, low-rank approximability, and attention perturbation response --- are essentially identical across transforms. The row-norm axis, not the global spectral profile, is where these two maps fundamentally differ.

The conditioning result refines the picture. The LN Jacobian bound $\|J_{\LN}\|_2\le 1/\sigma(x)$ \eqref{eq:ln-jacobian-norm} is loose precisely on rows with small variance, which we observe in DistilBERT's middle layers. The DyT Jacobian is bounded uniformly by $\alpha$. The empirical conditioning order --- DyT comparable to or better than LN through layers $1$--$5$ --- is consistent with the proven Jacobian bounds, and clarifies a subtle point: LN's exact invariances to shift and radial scale come at a conditioning cost when the data does not stay away from constant rows.

The convergence of effective rank, low-rank error, and attention perturbation response across transforms is informative. These quantities depend on the gross spectral envelope and on the structure of $QK^\top$, neither of which is strongly reshaped by a row-by-row map. Row-norm structure, by contrast, is a row-level property and is therefore exactly what differentiates LN from DyT.
