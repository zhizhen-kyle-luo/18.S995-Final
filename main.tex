...
All SVDs are computed in float64 on CPU. Per-head attention spectral metrics average over up to $32$ sequences and all $12$ heads; the perturbation test uses up to $16$ sequences.
...
Effective condition numbers do not simply favor LN. Figure~\ref{fig:kappa-X} shows $\kappaeff(X_{\ell,T})$ across all hidden states. DyT with $\alpha=1.0$ has lower $\kappaeff(X)$ than LN at every state except \textsf{layer\_5}, where LN happens to dip to a local minimum. In the middle layers ($\ell\in\{2,3,4\}$) the DyT-over-LN improvement is between $1.4\times$ and $2.0\times$. At \textsf{layer\_3}, $\kappaeff$ is $41712$ (raw), $30217$ (LN), and $15104$ (DyT $\alpha=1.0$).
...
\caption{Effective condition number $\kappaeff(X_{\ell,T})$ across hidden states. DyT $\alpha=1.0$ is better conditioned than LN at every state except \textsf{layer\_5}.}
\label{fig:kappa-X}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{Headline diagnostics at \textsf{layer\_6}. LN matches the predicted row isometry up to the $\varepsilon$ regularization; DyT $\alpha=1.0$ is best conditioned and has the highest effective rank; low-rank error and stable rank agree to within a few percent across all transforms.}
...
The relative Frobenius error of the best rank-$32$ approximation \eqref{eq:low-rank-error} is $0.336$ (raw), $0.363$ (LN), $0.360$ (DyT $\alpha=0.25$), $0.372$ (DyT $\alpha=0.5$), and $0.383$ (DyT $\alpha=1.0$): all five transforms lie within five percentage points of one another. Low-rank approximability is not a discriminating axis here. The Gram-diagonal diagnostic of Section~\ref{sec:results-gram} detects the row-geometry difference; rank-$32$ Frobenius error instead probes global spectral compressibility, and that is shared across the five matrices.
...
The conditioning result refines the picture. The LN Jacobian bound $\|J_{\LN}\|_2\le 1/\sigma(x)$ \eqref{eq:ln-jacobian-norm} is loose precisely on rows with small variance, which we observe in DistilBERT's middle layers. The DyT Jacobian is bounded uniformly by $\alpha$. The empirical conditioning order --- DyT better conditioned than LN at every hidden state except \textsf{layer\_5} --- is consistent with the proven Jacobian bounds and indicates that LN's exact invariances to shift and radial scale can coincide with worse conditioning when the data does not stay away from constant rows.

The relative agreement of low-rank error and attention perturbation response across transforms is informative; effective rank is also much closer across transforms than stable rank, with around $8$--$10\%$ spread at \textsf{layer\_6}, although the spread widens to around $20\%$ at \textsf{layer\_4} and \textsf{layer\_5}. These quantities depend on the gross spectral envelope and on the structure of $QK^\top$, neither of which is strongly reshaped by a row-by-row map. Row-norm structure, by contrast, is a row-level property and is therefore exactly what most cleanly differentiates LN from DyT in this experiment.
