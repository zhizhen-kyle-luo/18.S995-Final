...
Recent work has increasingly questioned which parts of normalization are actually necessary in transformers. Before transformers, Batch Normalization was the dominant approach, normalizing activations across the batch dimension. LayerNorm replaced it by normalizing within each example across the feature dimension, removing the dependence on batch statistics \cite{ba2016layer}. The original Transformer adopted LayerNorm from the start \cite{vaswani2017attention}. Subsequent work has probed how much of LayerNorm is essential: RMSNorm removes the mean-subtraction step while keeping rescaling, with little loss in performance \cite{zhang2019rmsnorm}; analyses of Pre-LN and Post-LN placement show that where LayerNorm sits affects gradient behavior and training stability; and NormFormer and DeepNet modify normalization or residual scaling to improve optimization and enable deeper models \cite{xiong2020layernorm,shleifer2021normformer,wang2022deepnet}. Dynamic Tanh (DyT) pushes this line of work further by replacing normalization layers entirely with the coordinatewise map
\begin{equation}
    \DyT(x)=\tanh(\alpha x),
    \qquad
...
This paper treats LayerNorm and DyT as numerical maps and asks which matrix-level properties they preserve. LayerNorm behaves as a rowwise projection-rescaling map: its local linearization removes shift and radial perturbation directions. DyT behaves as a coordinatewise saturation map: its Jacobian is diagonal and contracts saturated coordinates. We connect these structural differences to four diagnostics applied to \(X_\ell\), \(G_\ell\), and \(S_\ell\): effective condition number, stable rank, rank-$k$ approximation error, and a first-order perturbation bound on the attention scores. We then test these in one controlled DistilBERT case study. The goal is not to rank the two operations universally, but to separate task-level replacement from matrix-level equivalence.
...
\[
    ...
    \sum_{j=1}^d z_j
    =
    \sum_{j=1}^d \frac{x_j-\mu}{\sigma}
    =
    \frac{1}{\sigma}\sum_{j=1}^d (x_j-\mu)
    =
    \frac{d\mu - d\mu}{\sigma}
    =0.
\]
\end{proof}
...
\[
    ...
    =
    \frac{1}{\sigma^2}\sum_{j=1}^d (x_j-\mu)^2
    =
    \frac{d\sigma^2}{\sigma^2}
    =
    d,
\]
where the last equality uses \(\sigma^2 = \frac{1}{d}\sum_{j=1}^d(x_j-\mu)^2\).
\end{proof}
...
LayerNorm also has exact invariances. Adding \(a\one\) shifts the mean by \(a\) but leaves the centered vector unchanged:
\[
    (x+a\one)-\mu(x+a\one)\one
    =
    x-\mu(x)\one.
\]
Thus, for any \(a\in\R\),
...
\end{equation}
Multiplying by \(b\) scales the centered vector by \(b\) and the standard deviation by \(|b|\). Therefore, for any \(b>0\),
\begin{equation}
    \LN(bx)=\LN(x).
\label{eq:ln-scale-invariance}
\end{equation}
For \(b<0\), the same calculation gives \(\LN(bx)=-\LN(x)\). Thus LayerNorm discards global shift and positive radial scale, not merely locally but exactly.
...
\[
    \sigma(x)^2
    =
    \frac{1}{d}\sum_{j=1}^d (x_j-\mu(x))^2
    =
    \frac{\|c\|_2^2}{d},
    \qquad
...
\begin{proof}
Let \(N(u)=u/\|u\|_2\). The standard Jacobian of vector normalization is
\[
    J_N(u)
    =
    \frac{1}{\|u\|_2}
    \left(I-\frac{uu^\top}{\|u\|_2^2}\right).
\]
Now \(\LN(x)=\sqrt d\,N(Px)\). The inner map \(x\mapsto Px\) has Jacobian \(P\), and its value at \(x\) is \(c=Px\). The multivariable chain rule gives
\[
    J_{\LN}(x)
    =
    \sqrt d\,J_N(c)P
...
\]
Using \(\sigma(x)=\|c\|_2/\sqrt d\), this becomes
\[
    J_{\LN}(x)
    =
    \frac{1}{\sigma(x)}
    \left(I-\frac{cc^\top}{\|c\|_2^2}\right)P.
\]
Since \(P^\top=P\) and \(Pc=c\), we have \(c^\top P=c^\top\), so
\[
    \left(I-\frac{cc^\top}{\|c\|_2^2}\right)P
    =
    P-\frac{cc^\top}{\|c\|_2^2}
    =
    I-\frac{1}{d}\one\one^\top-\frac{cc^\top}{\|c\|_2^2},
\]
which gives \eqref{eq:ln-jacobian}. The null identities are checked directly:
\[
    \left(P-\frac{cc^\top}{\|c\|_2^2}\right)\one
    =
    P\one-\frac{c(c^\top\one)}{\|c\|_2^2}
    =
    0,
    \qquad
    \left(P-\frac{cc^\top}{\|c\|_2^2}\right)c
    =
    Pc-\frac{c(c^\top c)}{\|c\|_2^2}
    =
    c-c
    =
    0.
\]
Finally, let \(R=cc^\top/\|c\|_2^2\). Since \(c\in\one^\perp\), we have \(PR=RP=R\), and therefore
\[
    (P-R)^2
    =
    P^2-PR-RP+R^2
    ...
    (P-R)^\top=P-R.
\]
Thus \(P-R\) is an orthogonal projection, specifically onto \(\one^\perp\cap c^\perp\). Its eigenvalues are \(0\) or \(1\), so its spectral norm is at most \(1\), proving \eqref{eq:ln-jacobian-norm}.
\end{proof}
...
\begin{proof}
For each coordinate, the chain rule gives
\[
    \frac{d}{dt}\tanh(\alpha t)
    =
    \alpha\,\sech^2(\alpha t)
    =
    \alpha(1-\tanh^2(\alpha t)).
\]
Since the coordinates do not mix, the Jacobian is diagonal with entries in \([0,\alpha]\), so its spectral norm is at most \(\alpha\).
\end{proof}
...
\begin{proposition}[LayerNorm fixes the Gram diagonal]
...
\begin{proposition}[DyT bounds the Gram diagonal]
...
DyT does not remove row-magnitude variation. Its diagonal entries can vary across tokens according to how large the DyT-transformed rows are. For \(i\neq j\),
\[
...
\begin{proof}
Expanding \(\widetilde S-S\) gives the stated expression for \(\Delta S\). Hence
\[
    \|\Delta S\|_2
    \leq
    \frac{
        \|\Delta QK^\top\|_2+\|Q\Delta K^\top\|_2+\|\Delta Q\Delta K^\top\|_2
    }{\sqrt{d_k}}
    \leq
    \frac{
        \|\Delta Q\|_2\|K\|_2+\|Q\|_2\|\Delta K\|_2+\|\Delta Q\|_2\|\Delta K\|_2
    }{\sqrt{d_k}}.
\]
Dividing by \(\|S\|_2=\|QK^\top\|_2/\sqrt{d_k}\) gives \eqref{eq:attention-relative-perturbation}.
\end{proof}
